...
public:
	...
	void media_configure_helper(GstRTSPMediaFactory *factory, GstRTSPMedia *media)
	{
		...
		g_signal_connect(ctx->appsrc, "need-data", G_CALLBACK(start_feed), self);
		g_signal_connect(ctx->appsrc, "enough-data", G_CALLBACK(stop_feed), self);
	}
...
bool GStreamerOnvifRTSPSink::init()
{
	...
	mDetail->t1 = std::thread(&GStreamerOnvifRTSPSink::Detail::gstreamerMainThread, mDetail.get(), mDetail->loop);
