...
public:
    ...
    bool initNewVideo()
    {
        ...
        for (auto i = 0; i < mState.ntracks; i++)
        {
            ...
            if (mState.info.type == MP4_TRACK_TYPE_VIDEO && mState.videotrack == -1)
            {
                ...
                LOG_ERROR << "Total frames in video ==============================================================================>" << mState.mFramesInVideo;
            }
        }
    ...
    uint8_t * readNextFrame(size_t MetadataSize, size_t BufferSize, uint8_t* BFrames, uint8_t* MFrames)//, uint8_t* BFrames, uint8_t* MFrames
    {
        ...
        if (mState.has_more_video)
        {
            ret = mp4_demux_get_track_sample(mState.demux,
                ...
                &mState.sample);
            //image_buffer_size = sample_buffer_size;
           // metadata_buffer_actualsize = metadata_buffer_size;
...
{
    frame_sp bufferFrame;
    frame_sp metadataFrame;
	uint8_t *imageBuffer;
    imageBuffer = bufferFrame = makeFrame(mDetail->mProps.BufferSize, getOutputPinIdByType(FrameMetadata::ENCODED_IMAGE));
    metadataFrame = makeFrame(mDetail->mProps.MetadataSize, getOutputPinIdByType(FrameMetadata::MP4_VIDEO_METADATA));
    uint8_t *BFrames = reinterpret_cast<uint8_t *>(bufferFrame->data());
    uint8_t *MFrames = reinterpret_cast<uint8_t *>(metadataFrame->data());
	...
	imageBuffer = mDetail->readNextFrame(mDetail->mProps.MetadataSize, mDetail->mProps.BufferSize, BFrames, MFrames, ); // remove MBuffer and BFrame argument from readNextFrame func//, BFrames, MFrames
    ...
    auto frame1 = makeFrame(bufferFrame, mDetail->mProps.BufferSize, encodedImagePinId);
    frame_container frames;
 
    frames.insert(make_pair(encodedImagePinId, frame1));
 
    auto metadataBufferSize = mDetail->mProps.MetadataSize;
    if (metadataBufferSize)
    {
        // #Dec_27_Review - redundant use makeBuffer to readNextFrame
        auto metadataFrames = makeFrame(metadataFrame, metadataBufferSize, getOutputPinIdByType(FrameMetadata::MP4_VIDEO_METADATA));
 
        frames.insert(make_pair(mp4FramePinId, metadataFrames));
    }
