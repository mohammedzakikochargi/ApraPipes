...
public:
	...
	start_pipeline(void)
	{
		...
		pipe1 = gst_parse_launch("appsrc is-live=TRUE block=TRUE name=gstsrc min-latency=0 ! videoconvert ! video/x-raw, format=I420 ! x264enc tune=zerolatency speed-preset=superfast ! rtph264pay ! queue ! application/x-rtp,media=video,encoding-name=H264,payload=96 ! webrtcbin name=sendrecv", &error);
		...
		// 											 "gstsrc");
		ctx->appsrc = gst_bin_get_by_name(GST_BIN(pipe1),
										  "gstsrc");
		...
		LOG_INFO << "Width" << mProps.width << "Height" << mProps.height;
		...
		g_signal_connect(uridb1, "element-added",
						 G_CALLBACK(uridecodebin_element_added), this);
...
public:
	...
	GMainLoop *loop;
	SoupMessage *message;
	GstElement *pipe1, *webrtc1, *uridb1;
								  ...
								  void *userdata)
{
	...
	detail->on_incoming_decodebin_stream_helper(decodebin, pad, detail->pipe1);
	return;
}
...
{
	auto detail = static_cast<GStreamerWebRTCSink::Detail *>(userdata);
	detail->on_incoming_stream_helper(webrtc, pad, detail->pipe1);
	return;
}
